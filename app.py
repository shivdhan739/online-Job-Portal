...
app.config['MYSQL_PASSWORD'] = "Shiv@123"
...
def jobs():
    if "user" in session:
        if request.method == 'POST':
            ...
            if keyword and not location:
                count_search = cur.execute("""
                    ...
                    WHERE job.job_title LIKE %s 
                       OR job.job_type LIKE %s 
            ...
            jobsearch = cur.fetchall() if count_search > 0 else []
            return render_template('jobsearch.html', jobsearch=jobsearch)
        ...
        alljobs = cur.fetchall() if count_jobs > 0 else []
        return render_template('jobs.html', alljobs=alljobs)
...
def apply():
    if "user" in session:
        user = session['user']  # Assuming user is jobseeker_id
        if request.method == 'POST':
            form = request.form
            job_id = form['j_id']  # Job ID from the HTML form
            cur = mysql.connection.cursor()

            # Check if the user already applied to this job
            cur.execute("SELECT * FROM apply WHERE jobseeker_id = %s AND job_id = %s", (user, job_id))
            applied = cur.fetchone()

            if not applied:
                # Insert a new row in apply table
                applied_on = datetime.now()
                cur.execute(
                    "INSERT INTO apply (job_id, jobseeker_id, applied_on) VALUES (%s, %s, %s)",
                    (job_id, user, applied_on)
                )
                mysql.connection.commit()
            cur.close()
        return redirect(url_for('home'))
